...
test('should verify a correct password with pbkdf2', async t => {
  const hash = await m.hash('hello world', {func: 'pbkdf2'});
...
test.serial('should throw an error trying to hash a non valid string', async t => {
  let err = await t.throws(m.hash(undefined, {func: 'pbkdf2'}));
  t.true(err instanceof Error);
  err = await t.throws(m.hash('', {func: 'pbkdf2'}));
  t.true(err instanceof Error);
  err = await t.throws(m.hash(['unicorn'], {func: 'pbkdf2'}));
  t.true(err instanceof Error);
  err = await t.throws(m.hash(() => console.log('lalala'), {func: 'pbkdf2'}));
  t.true(err instanceof Error);
  err = await t.throws(m.hash(null, {func: 'pbkdf2'}));
  t.true(err instanceof Error);
});
